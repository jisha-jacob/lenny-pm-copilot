...
| Answer prompt choice | Keep the current fallback-aware prompt (`answer.SYSTEM_PROMPT`) as the default — do not switch to the plain variant | Decided in issue #20. Issue #10's single run scored plain 5.00/5.00 vs. fallback-aware 5.00/4.95 (groundedness/relevance) — a margin too thin to call. Re-ran `eval_llm.py`'s comparison 3 more times (4 total runs including #10's): groundedness is a wash both ways (plain 4.975 avg, fallback-aware 4.9875 avg, direction flips run to run); relevance is a small but *consistent* gap in the plain prompt's favor across all 4 runs (plain 4.9875 avg vs. fallback-aware 4.8625 avg — fallback-aware never once won on relevance). So the margin is real, not noise, on the relevance dimension specifically. Kept the fallback-aware prompt anyway, because the eval'd "plain" variant that scored higher is not actually a valid swap-in: it drops the explicit "if the excerpts don't contain enough information, say so plainly instead of guessing" instruction that issue #5's acceptance criteria require, and that instruction was never itself re-tested in a version that also kept the not-enough-info wording. The ~0.13-point relevance cost (on a golden set of by-design-answerable questions) is treated as an accepted trade for that explicit safety instruction, which is defense-in-depth alongside — not a replacement for — the separate retrieval-confidence fallback (issue #9). Revisit if a version of the plain prompt that keeps the not-enough-info instruction is ever eval'd and shows a real, larger margin. |
